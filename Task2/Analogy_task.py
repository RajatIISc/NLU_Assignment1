...
def nearest_k(word1,word2,word3,k):
    s=list()
    i=word2int[word1]
    j=word2int[word2]
    l=word2int[word3]
    query_vector=E[i]
    for item in words:
        vec=E[word2int[item]]-E[j]+E[l]
        s.append([euclidean_dist(query_vector,vec),word2int[item]])
    sorted_list=sorted(s,key = sortFirst)
    print("top-%d closest words\n"%k)
   
    for i in range(k):
        a=int2word[sorted_list[i][1]]
        print(a)    
        
            
               
def bias_nearest_k(word1,k):
    s=list()
    i=word2int[word1]
    #j=word2int[word2]
    #l=word2int[word3]
    ...
    for item in words:
        #vec=E[word2int[item]]-E[j]+E[l]
        s.append([euclidean_dist(query_vector,E[word2int[item]]),word2int[item]])
    sorted_list=sorted(s,key = sortFirst)
    print("top-%d closest words\n"%k)
